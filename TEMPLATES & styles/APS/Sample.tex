...
convenient to install a ``complete \TeX\ distribution,'' which will 
include \LaTeX, the underlying \TeX\ engine, macro packages such as
REV\TeX, a large collection of fonts, and GUI tools for editing and
viewing your documents.  To test your installation, try to process
this sample article.
...
wherever
you
want, and don't      worry      about      extra      spaces      between      words, which \LaTeX\ will ignore.  You can almost always trust \LaTeX\ to make your paragraphs look good, with neatly justified margins.  % Long lines of text are fine, since most editors will wrap them automatically.
...
expressions, even when they don't contain any special symbols.  But don't 
use math mode to italicize ordinary \textit{words}.
...
This example also shows how to make the sum and integral symbols, big 
parentheses, and built-up fractions.  (Don't put built-up fractions in a 
...
AJP prefers that raster images be in .tiff format. \LaTeX\ can't 
import .tiff images, so we recommend using .png or .jpg with \LaTeX\ 
